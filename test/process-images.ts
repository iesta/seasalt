...
async function processImage(imagePath: string): Promise<Card[]> {
  ...
  const parsed = extractJson(content) as DetectedCardRaw
...
async function main() {
  if (!API_KEY) {
    console.error('❌ OPENROUTER_KEY non définie. Exporte-la : export OPENROUTER_KEY=sk-or-v1-...')
    process.exit(1)
  }
  ...
  if (files.length === 0) {
    console.error('❌ Aucune image trouvée dans test/. Ajoute des .png/.jpg/.webp.')
    process.exit(1)
  }

  console.log(`📸 ${files.length} image(s) à traiter avec le modèle ${MODEL}\n`)

  const csvHeader = 'image,card_count,score,dominant_color,dominant_count'
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    try {
      ...
      console.log(`  ✅ ${cards.length} cartes | Score: ${result.total} | Dominant: ${dominant?.label ?? '—'} (${dominant?.count ?? 0})`)
    } catch (e) {
